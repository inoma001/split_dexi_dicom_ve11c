...
function [images, headers, nFiles] = dicomfolder(folderName,mosaic)
...
for i = 1:numImgs
    ...
    %need to switch depending on mosaic or not
  	if mosaic <1
        sortTable(:,3) = cellfun(@(x) x.('ImagePositionPatient')(3), headersRaw(imgFlag), 'uni', 0); %for non-mosic images
    else
        sortTable(:,3) = cellfun(@(x) x.('AcquisitionNumber'), headersRaw(imgFlag), 'uni', 0); %for mosaic images
    end
